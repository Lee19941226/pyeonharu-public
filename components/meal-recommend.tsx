...
import {
  Sparkles, Store, Flame, Youtube,
  ChevronDown, ChevronUp, MapPin, Loader2,
  UtensilsCrossed, AlertTriangle, Bike
} from "lucide-react"
...
export default function MealRecommend() {
  ...
  // 칼로리 동기화: 기존 /api/diet/entries 사용
    ...
    <div className="rounded-2xl border bg-card shadow-sm overflow-hidden">
      ...
      <div className="flex items-center justify-between px-3 py-2.5 border-b bg-gradient-to-r from-primary/5 to-amber-50/50">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[9px] text-muted-foreground">내 알레르기·식단 기반</p>
          </div>
        </div>
        <button
        ...
        <div className="flex items-center gap-2 px-3 py-2 border-b bg-muted/30">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between text-[10px]">
              ...
              <span className="font-medium">잔여 {remainingCal.toLocaleString()}kcal</span>
            </div>
            <div className="mt-0.5 h-1 rounded-full bg-muted overflow-hidden">
              <div
                className={`h-full rounded-full transition-all ${todayCal > bmr ? "bg-red-400" : "bg-gradient-to-r from-green-400 to-amber-400"}`}
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center py-8 gap-1.5">
          ...
          <p className="text-[10px] text-muted-foreground">식단 분석 중...</p>
        </div>
      )}
