...
export default function DietTab() {
  ...
  const handleManualSubmit = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        toast.success(`📝 ${manualName} (${manualCal}kcal)`);
        setManualName("");
        setManualCal("");
  ...
  return (
    <div className="w-full">
          ...
          <div className="w-full max-w-md space-y-3">
            ...
            {isToday && (
              <div className="flex gap-2">
                <Button
                  variant="outline"
                  className="flex-1 gap-1.5 text-xs h-9 border-primary/30 text-primary hover:bg-primary/5"
                          ...
                          <div className="flex gap-2 flex-1 min-w-0">
                            {entry.image_url ? (
                              <button
                                ...
                                className="flex-shrink-0 overflow-hidden rounded-lg border hover:border-primary/50"
                              >
                                <img
                                  src={entry.image_url}
                                  alt={entry.food_name}
                              ...
                              <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-muted text-sm">
                                {entry.emoji}
                              </div>
                            )}
      ...
      {showManualInput && (
        ...
        >
          <div
            className="w-full max-w-sm rounded-xl bg-background p-5 space-y-4"
          ...
          >
            ...
            <div className="rounded-lg bg-blue-50 p-2.5">
              <p className="text-xs text-blue-700">
                💡 메뉴에 칼로리가 표기된 경우 직접 입력하면 더 정확해요.
              </p>
            </div>
            <div className="flex gap-2">
              <Button
              ...
              <Button
                className="flex-[2]"
                disabled={
                  !manualName.trim() || !manualCal || parseInt(manualCal) <= 0
                }
