...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    console.error("[reports/weekly]", error.message);
    return NextResponse.json(
      { error: "서버 오류가 발생했습니다." },
      { status: 500 },
    );
  ...
  const safeRate =
    totalScans > 0 ? Math.round((safeScans / totalScans) * 100) : 0;
  ...
  const foodCountMap: Record<
    string,
    { foodName: string; manufacturer: string; count: number; isSafe: boolean }
  > = {};
  ...
  const dailyStats: Record<
    string,
    { total: number; safe: number; danger: number }
  > = {};
  for (let i = 0; i < 7; i++) {
