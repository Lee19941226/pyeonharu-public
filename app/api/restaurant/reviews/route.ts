...
export async function GET(req: NextRequest) {
  ...
  if (keys) {
    ...
    let myReviews: Record<
      string,
      { id: string; rating: number; memo: string }
    > = {};
    if (user) {
  ...
  // ── 단일 음식점 리뷰 목록 ──
  const targetKey =
    key || (name ? makeRestaurantKey(name, address || "") : null);
  if (!targetKey) {
    return NextResponse.json(
      { error: "key 또는 name 파라미터가 필요합니다." },
      { status: 400 },
    );
  }
  ...
  if (reviewError) {
    console.error("[restaurant/reviews]", reviewError.message);
    return NextResponse.json(
      { error: "서버 오류가 발생했습니다." },
      { status: 500 },
    );
