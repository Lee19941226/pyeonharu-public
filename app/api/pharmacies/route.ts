...
export async function GET(request: NextRequest) {
  ...
  try {
    // 올바른 엔드포인트: getParmacyLcinfoInqire (위치정보 조회)
    const baseUrl = "https://apis.data.go.kr/B552657/ErmctInsttInfoInqireService/getParmacyLcinfoInqire"
    
    // 파라미터 구성
    const params = new URLSearchParams()
    params.append("serviceKey", SERVICE_KEY)
    params.append("WGS84_LON", lng)  // 경도
    params.append("WGS84_LAT", lat)  // 위도
...
function parsePharmacyXML(xml: string): Array<{
  ...
  while ((match = itemRegex.exec(xml)) !== null) {
    ...
    // 올바른 필드명: latitude, longitude (wgs84Lat/wgs84Lon 아님!)
    const latitude = parseFloat(getValue("latitude")) || 0
    const longitude = parseFloat(getValue("longitude")) || 0
