...
async function enrichPosts(supabase: any, data: any[], userId?: string) {
  ...
  if (schoolCodes.length > 0) {
    const { data: schools } = await supabase
      .from("user_schools").select("school_code, school_name").in("school_code", schoolCodes)
    for (const s of schools || []) {
      if (!schoolNameMap[s.school_code]) schoolNameMap[s.school_code] = s.school_name
    }
  }
...
export async function POST(req: NextRequest) {
  ...
  const { data: schoolCheck } = await supabase
    .from("user_schools").select("id")
    .eq("user_id", user.id).eq("school_code", schoolCode).maybeSingle()

  if (!schoolCheck) {
    return NextResponse.json({ error: "등록된 학교에서만 글을 작성할 수 있습니다." }, { status: 403 })
  }
