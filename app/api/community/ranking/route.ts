...
export async function GET(req: NextRequest) {
  ...
  // 학교별 집계
  const schoolMap = new Map<string, { posts: number; likes: number; views: number; comments: number }>()

  for (const post of recentPosts || []) {
    const code = post.school_code
    const cur = schoolMap.get(code) || { posts: 0, likes: 0, views: 0, comments: 0 }
    cur.posts += 1
  ...
  const ranking = Array.from(schoolMap.entries())
    .map(([code, stats]) => ({
      ...
      score: stats.posts * 10 + stats.comments * 3 + stats.likes * 5 + Math.floor(stats.views * 0.1),
    }))
    .sort((a, b) => {
