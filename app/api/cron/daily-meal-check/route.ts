...
function parseMenuItem(raw: string): ParsedMenuItem {
  ...
  if (parenMatch) {
    ...
    const names = numbers.map(n => NEIS_ALLERGEN_MAP[n] || "").filter(Boolean)
    return { name, allergenNumbers: numbers, allergenNames: names }
  }
  const match = cleaned.match(/^(.+?)[\s*]*([\d]+(?:\.[\d]+)*\.?)$/)
  if (match) {
    const name = match[1].replace(/\*$/, "").trim()
    const numbers = match[2].split(".").filter(n => n.trim() !== "")
    const names = numbers.map(n => NEIS_ALLERGEN_MAP[n] || "").filter(Boolean)
    return { name, allergenNumbers: numbers, allergenNames: names }
  }
...
function buildEmailHTML(
  ...
  mealTypeName: string,
  calInfo: string,
  menuItems: { name: string; status: "safe" | "danger"; matchedAllergens: string[] }[],
  hasDanger: boolean,
) {
  const dangerItems = menuItems.filter(m => m.status === "danger")
  const safeItems = menuItems.filter(m => m.status === "safe")
  ...
  const dangerHTML = dangerItems.length > 0
  ...
  const safeHTML = safeItems.length > 0
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (dayOfWeek === 0 || dayOfWeek === 6) {
      return NextResponse.json({ message: "주말에는 급식 알림을 보내지 않습니다.", sent: 0 })
    }
    ...
    for (const [userId, schools] of userSchoolMap) {
      try {
        ...
        const userAllergenNames = (userAllergens || []).map(a => a.allergen_name)
        ...
        for (const school of schools) {
          try {
            ...
            for (const row of rows) {
              ...
              // 알레르기 매칭
              const matchedMenu = parsedMenu.map(item => {
                const matched = item.allergenNames.filter(name =>
                  userAllergenNames.some(ua => name.includes(ua) || ua.includes(name))
                )
                return {
                  name: item.name,
                  status: (matched.length > 0 ? "danger" : "safe") as "safe" | "danger",
                  matchedAllergens: matched,
                }
              })

              const hasDanger = matchedMenu.some(m => m.status === "danger")
            ...
            console.error(`[Cron] 학교 급식 조회 실패: ${school.school_name}`, e)
          }
        }
        ...
        const hasAnyDanger = schoolResults.some(r => r.hasDanger)
        const subject = hasAnyDanger
          ? `⚠️ [편하루] 오늘 급식에 알레르기 주의 메뉴가 있어요!`
          : `✅ [편하루] 오늘 급식은 안전해요!`

        // 여러 학교면 합쳐서 보냄
        const htmlParts = schoolResults.map(r =>
          buildEmailHTML(r.schoolName, r.mealTypeName, r.calInfo, r.menuItems, r.hasDanger)
        )

        // 여러 학교를 하나의 이메일로 합침
        const combinedHTML = htmlParts.length === 1
          ? htmlParts[0]
          : `
<!DOCTYPE html>
<html>
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:520px;margin:0 auto;padding:20px;color:#1F2937;">
  ...
  ${schoolResults.map(r => {
    const dangerItems = r.menuItems.filter(m => m.status === "danger")
    const safeItems = r.menuItems.filter(m => m.status === "safe")
    ...
    <div style="padding:20px 0;border-bottom:1px solid #F3F4F6;">
      ...
      ${dangerItems.length > 0 ? `<div style="padding:8px 12px;background:#FEF2F2;border-radius:6px;margin-bottom:8px;">
        ${dangerItems.map(d => `<p style="margin:3px 0;font-size:13px;color:#991B1B;">🚨 <strong>${d.name}</strong> — ${d.matchedAllergens.join(", ")}</p>`).join("")}
      </div>` : ""}
      <div style="padding:8px 12px;background:#F0FDF4;border-radius:6px;">
        ${safeItems.map(s => `<p style="margin:3px 0;font-size:13px;color:#166534;">✅ ${s.name}</p>`).join("")}
      </div>
    </div>`
  }).join("")}
  <div style="padding:16px 0;text-align:center;">
    ...
    return NextResponse.json({ error: "급식 알림 처리 중 오류가 발생했습니다." }, { status: 500 })
  }
}
