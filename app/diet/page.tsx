...
import { Camera, ImageIcon, PenLine, Trash2, X, AlertTriangle, Activity, ChevronLeft, ChevronRight, Loader2, Info, Sparkles, CalendarDays, TrendingUp, TrendingDown, Flame, BarChart3, Trophy, Utensils, Download, Share2, Plus } from "lucide-react"
...
function DietPageInner() {
  ...
  useEffect(() => {
    const handler = () => { if (user) loadEntries() }
    window.addEventListener("diet-entry-added", handler)
    return () => window.removeEventListener("diet-entry-added", handler)
  }, [user, loadEntries])

  // ✅ URL 쿼리 파라미터 refresh 감지
  useEffect(() => {
    const refreshParam = searchParams.get("refresh")
    if (refreshParam && user) { loadEntries(); router.replace("/diet", { scroll: false }) }
  }, [searchParams, user])
          ...
          {viewMode === "daily" && (<>
            ...
            {isToday && (
              <div className="flex gap-2">
                <Button className="flex-1 gap-2" onClick={() => setShowRecordSheet(true)} disabled={isAnalyzing}>
                  {isAnalyzing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />}
                  {isAnalyzing ? "분석 중..." : "기록하기"}
                </Button>
                <div className="relative">
                  <Button variant="outline" size="icon" className="border-primary/30 text-primary hover:bg-primary/5" onClick={() => setShowShareMenu(!showShareMenu)} disabled={entries.length === 0}>
                    <Share2 className="h-4 w-4" />
                  </Button>
                  {showShareMenu && entries.length > 0 && (
                    <>
                      <div className="fixed inset-0 z-40" onClick={() => setShowShareMenu(false)} />
                      <div className="absolute right-0 top-11 z-50 w-52 rounded-xl border bg-background shadow-lg p-1.5 space-y-0.5">
                        <button onClick={shareToday} className="w-full flex items-center gap-3 rounded-lg px-3 py-2.5 text-xs hover:bg-muted transition-colors text-left">
                          <span className="text-lg">🍽️</span>
                          <div><p className="font-medium">먹은 음식 공유</p><p className="text-[10px] text-muted-foreground">오늘 먹은 것들 카카오톡으로</p></div>
                        </button>
                        <button onClick={shareReport} className="w-full flex items-center gap-3 rounded-lg px-3 py-2.5 text-xs hover:bg-muted transition-colors text-left">
                          <span className="text-lg">📊</span>
                          <div><p className="font-medium">식단 리포트 공유</p><p className="text-[10px] text-muted-foreground">칼로리 요약 · 통계</p></div>
                        </button>
                      </div>
                    </>
                  )}
            ...
            {isLoading ? <div className="space-y-3">{[...Array(3)].map((_, i) => <Skeleton key={i} className="h-16 w-full rounded-lg" />)}</div> : entries.length === 0 ? <div className="py-12 text-center"><p className="text-sm text-muted-foreground">아직 기록이 없어요</p>{isToday && <p className="text-xs text-muted-foreground mt-1">위 기록하기 버튼으로 음식을 추가해보세요</p>}</div> : (
          ...
          {viewMode !== "daily" && (<>
            ...
            {!reportLoading && reportData && (
              <div className="flex justify-end">
                <Button variant="outline" size="sm" className="gap-1.5 text-xs border-primary/30 text-primary hover:bg-primary/5" onClick={shareReport}>
                  <Share2 className="h-3.5 w-3.5" />리포트 공유
                </Button>
              </div>
            )}
            {reportLoading ? <div className="space-y-4"><Skeleton className="h-32 w-full rounded-lg" /><Skeleton className="h-48 w-full rounded-lg" /><Skeleton className="h-24 w-full rounded-lg" /></div> : !reportData ? <div className="py-12 text-center"><p className="text-sm text-muted-foreground">데이터를 불러오는 중...</p></div> : (<>
...
export default function DietPage() {
  return (
    <Suspense fallback={<div className="flex min-h-screen flex-col bg-background"><Header /><main className="flex-1 pb-16 md:pb-0"><div className="container mx-auto px-4 py-8"><div className="mx-auto max-w-lg text-center py-12"><Loader2 className="h-8 w-8 animate-spin text-primary mx-auto" /><p className="text-sm text-muted-foreground mt-3">식단 관리를 불러오는 중...</p></div></div></main><MobileNav /></div>}>
      <DietPageInner />
    </Suspense>
  )
}
