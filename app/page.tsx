...
export default function HomePage() {
  ...
  const handleFoodSearch = () => {
    if (foodInput.trim()) {
      router.push(`/food?q=${encodeURIComponent(foodInput)}`);
    }
  };
                ...
                {searchMode === "search" && (
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-2">
                      <Button
                        variant={
                          placeType === "hospital" ? "default" : "outline"
                        }
                        size="sm"
                      ...
                      <Button
                        variant={
                          placeType === "pharmacy" ? "default" : "outline"
                        }
                        size="sm"
                    ...
                    {placeLoading ? (
                      ...
                      <div className="space-y-2">
                        <p className="text-xs text-muted-foreground">
                          총 {placeTotalCount.toLocaleString()}건 중{" "}
                          {allPlaces.length.toLocaleString()}건 로드됨
                        </p>
                    ...
                    {hasMore && !placeKeyword && (
                      <div className="pt-2 text-center">
                        ...
                        >
                          {placeLoading
                            ? "불러오는 중..."
                            : `다음 1000개 불러오기 (${allPlaces.length.toLocaleString()} / ${placeTotalCount.toLocaleString()})`}
            ...
            <CardContent className="space-y-4">
              {loading ? (
                ...
                <>
                    ...
                    <div className="mt-3 flex gap-2">
                      {showCount < 100 && diseases.length > showCount && (
                        <Button
                          ...
                          className="flex-1"
                          onClick={() =>
                            setShowCount(Math.min(showCount + 40, 100))
                          }
                        >
                          더보기 ({showCount}위 →{" "}
                          {Math.min(showCount + 40, 100)}위)
                          <ChevronDown className="ml-1 h-4 w-4" />
                        </Button>
                      )}
                      {showCount > 10 && (
                  ...
                  <p className="text-xs text-muted-foreground">
                    * 데이터 출처: 질병관리청 감염병포털 · 클릭하면 상세 정보를
                    확인할 수 있습니다
                  </p>
                </>
              )}
