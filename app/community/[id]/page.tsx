...
import {
  ArrowLeft, Heart, MessageCircle, Eye, Trash2, Edit, Send,
  CornerDownRight, Loader2, MoreHorizontal, Image as ImageIcon, GraduationCap,
} from "lucide-react"
...
export default function PostDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleLikeComment = async (commentId: string) => {
    ...
    try {
      ...
      const updateComments = (list: Comment[]): Comment[] =>
        list.map(c => {
          if (c.id === commentId) {
            return { ...c, isLiked: data.liked, likeCount: c.likeCount + (data.liked ? 1 : -1) }
          }
  ...
  const handleDeleteComment = async (commentId: string) => {
    ...
    try {
      const res = await fetch(`/api/community/${id}/comments?commentId=${commentId}`, { method: "DELETE" })
      if (res.ok) {
        // 1단에서 삭제
        const newComments = comments.filter(c => c.id !== commentId).map(c => ({
          ...c,
  ...
  if (isLoading) {
    return (
      <div className="flex min-h-screen flex-col bg-background">
        <Header />
        <main className="flex-1 pb-20"><div className="container mx-auto px-4 py-6"><div className="mx-auto max-w-2xl space-y-4">
      ...
      <main className="flex-1 pb-20 md:pb-0">
        <div className="container mx-auto px-4 py-4">
          <div className="mx-auto max-w-2xl space-y-4">
            ...
            {/* 뒤로가기 */}
            <Button variant="ghost" size="sm" onClick={() => router.push("/community")} className="gap-1.5">
                ...
                <div className="flex items-center justify-between border-t pt-3">
                  ...
                  {post.isOwner && (
                    <div className="flex gap-1">
                      <Button variant="ghost" size="sm" onClick={() => router.push(`/community/write?edit=${id}`)}>
            ...
            {/* 댓글 섹션 */}
            <div>
              <h2 className="mb-3 text-sm font-semibold">댓글 {post.comment_count}개</h2>
                ...
                {comments.map(comment => (
                  <div key={comment.id}>
                    ...
                    <CommentItem
                      ...
                      onReply={() => { setReplyTo({ id: comment.id, author: comment.author }); setCommentInput("") }}
              ...
              {user ? (
                <div className="mt-4 space-y-2">
                  {replyTo && (
                    <div className="flex items-center gap-2 text-xs text-muted-foreground">
                      ...
                      <button onClick={() => setReplyTo(null)} className="text-destructive hover:underline">취소</button>
                    </div>
                  )}
                  <div className="flex gap-2">
                    <Textarea
                      placeholder={replyTo ? `${replyTo.author}님에게 답글 작성...` : "댓글을 작성하세요..."}
                      value={commentInput}
                      onChange={(e) => setCommentInput(e.target.value)}
                      className="min-h-[60px] resize-none text-sm"
                      onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleSubmitComment() } }}
                    />
                    <Button size="icon" onClick={handleSubmitComment} disabled={submitting || !commentInput.trim()}>
                ...
                <Card className="mt-4 border-dashed">
                  <CardContent className="flex items-center justify-between p-3">
                    <p className="text-xs text-muted-foreground">댓글을 작성하려면 로그인하세요</p>
...
function CommentItem({
  comment, isReply, onLike, onReply, onDelete,
}: {
  comment: Comment
  isReply?: boolean
  onLike: () => void
  ...
  return (
    <div className={`rounded-lg border p-3 ${isReply ? "bg-muted/30" : ""}`}>
      ...
      <div className="flex items-center gap-3 text-xs text-muted-foreground">
        <button onClick={onLike} className={`flex items-center gap-0.5 hover:text-red-500 ${comment.isLiked ? "text-red-500" : ""}`}>
          <Heart className={`h-3 w-3 ${comment.isLiked ? "fill-red-500" : ""}`} />
          {comment.likeCount > 0 && comment.likeCount}
        </button>
        {!isReply && onReply && (
          <button onClick={onReply} className="flex items-center gap-0.5 hover:text-foreground">
