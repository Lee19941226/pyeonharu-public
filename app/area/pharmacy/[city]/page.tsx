...
import { Cross, MapPin, Phone, Search, Navigation, ChevronRight } from "lucide-react"
...
async function getPharmacies(sidoCd: string) {
  try {
    ...
    return { pharmacies: data.pharmacies || [], totalCount: data.totalCount || 0 }
  } catch (error) {
          ...
          <div className="mx-auto max-w-3xl">
            ...
            {pharmacies.length === 0 ? (
              <Card>
                <CardContent className="py-12 text-center">
                  ...
                  <p className="text-muted-foreground">등록된 약국이 없습니다.</p>
                </CardContent>
              </Card>
            ) : (
                    ...
                    <CardContent className="p-4">
                      <div className="mb-3">
                        ...
                        <div className="mt-1 flex flex-wrap gap-1">
                          {pharmacy.sgguCdNm && <Badge variant="outline">{pharmacy.sgguCdNm}</Badge>}
                          {pharmacy.emdongNm && <Badge variant="secondary">{pharmacy.emdongNm}</Badge>}
                        </div>
                      </div>

                      <div className="mb-4 space-y-1 text-sm text-muted-foreground">
                        <p className="flex items-center gap-2">
                          <MapPin className="h-4 w-4 shrink-0" />{pharmacy.address}
                        </p>
                        {pharmacy.phone && (
                          <p className="flex items-center gap-2">
                            <Phone className="h-4 w-4 shrink-0" />{pharmacy.phone}
                          </p>
                        )}
                      </div>
                      ...
                      <div className="flex gap-2">
                        {pharmacy.phone && (
                          <Button variant="outline" size="sm" className="flex-1" asChild>
                            <a href={`tel:${pharmacy.phone}`}><Phone className="mr-1 h-4 w-4" />전화</a>
                          </Button>
                        )}
                        {pharmacy.lat > 0 && (
                          <Button variant="outline" size="sm" className="flex-1" asChild>
                            <a href={`nmap://route/public?dlat=${pharmacy.lat}&dlng=${pharmacy.lng}&dname=${encodeURIComponent(pharmacy.name)}`} target="_blank">
            ...
            <div className="mt-8 rounded-lg bg-primary/5 p-4">
              <p className="text-sm">
                ...
                <Link href={`/area/hospital/${city}`} className="font-medium text-primary hover:underline">
            ...
            <div className="mt-8">
              ...
              <div className="flex flex-wrap gap-2">
                {Object.values(REGIONS).filter(r => r.slug !== city).slice(0, 8).map((r) => (
                  <Link key={r.slug} href={`/area/pharmacy/${r.slug}`} className="text-sm text-green-600 hover:underline dark:text-green-400">
                    {r.name}
                  </Link>
                ))}
              </div>
